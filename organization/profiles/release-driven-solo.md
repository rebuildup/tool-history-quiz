...
tool-history-quiz の current release-driven development で使用する Operating Model を定義します。
...
- GitHub Issues: durable implementation scope / acceptance criteria / dependency state
- this repository does not currently use Linear as a canonical planning surface
- host repository work state is not mirrored here as a second canonical source
...
- Bun / Vite / React / TypeScript / Biome decisions in existing ADRs remain project-specific toolchain authority.
- `@rebuildup/my-web-tools-ui` is a sibling host dependency; standalone validation and host-layout integration evidence are distinct.
- branch protection requires CI + conversation resolution, not solo self-approval.
- repository landing is merge-commit-only; squash/rebase merge are disabled.
- current release line is `release-x-y-z`; main receives normal integration only from the release PR.

## Agent Skills lifecycle

- project-init operational Skills track current `rebuildup/project-init` through `bunx skills`.
- canonical install layout: `.agents/skills/` plus `.claude/skills/` for Claude Code.
- fresh setup: `mise run skills-bootstrap`.
- continuous update: `mise run skills-update` / `bunx skills update -p -y`.
- `skills-lock.json` is committed durable source / skill path / content-hash metadata.
- `--global` is not repository freshness evidence.
